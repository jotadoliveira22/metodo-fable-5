...
**Entender.** Objetivo real: que `/orders` vuelva a funcionar y no reincida.
Entregable: corrección desplegable + causa raíz + prueba de regresión.
Dato clave dado: "desde ayer" → algo cambió ayer.

**Presupuesto por costo del error.** Producción degradada → **fila 4 (grave/irreversible)**:
verificación completa, revisión adversarial, confirmación humana en pasos irreversibles
y plan de reversa.

**Evidencia de éxito.** `GET /orders` devuelve 200 con datos correctos en los casos que hoy fallan;
una prueba cubre el caso; se explica la causa raíz.

**Generar: evidencia antes de tocar nada.**
...
**Ejecutar incremental: cambio mínimo.** Manejar `invoice == null` en el serializador (devolver `total: null`
...
**Auditar: revisión adversarial.** ¿Otros endpoints asumen `invoice` no nulo? → grep de
